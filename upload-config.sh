...
# VSCode 配置上传脚本 - 将本地配置推送到远程仓库
# 此脚本用于将本地VSCode配置更新推送到远程仓库
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --push-remotes)
            ...
            ;;
        --sync-before)
            SYNC_BEFORE_UPDATE=true
            shift
            ;;
        --help)
            echo -e "${BOLD}VSCode配置上传脚本${NC}"
            echo ""
            echo "用法: $0 [选项]"
            echo ""
            echo "选项:"
...
# 主程序开始
echo -e "${BOLD}${CYAN}📤 VSCode配置上传脚本${NC}"
echo -e "${GRAY}════════════════════════════════════════${NC}"
...
# 上传前先同步远程更改
if [ "$SYNC_BEFORE_UPDATE" = true ]; then
    echo ""
    echo -e "${BOLD}${YELLOW}🔄 同步远程更改...${NC}"
    ...
    if git fetch origin "$current_branch"; then
        ...
        print_warning "⚠ 获取远程更改失败，继续上传本地更改"
    fi
fi
...
# 完成信息
echo ""
echo -e "${BOLD}${GREEN}🎉 VSCode配置上传完成！${NC}"
echo -e "${GRAY}════════════════════════════════════════${NC}"
...
echo ""
echo -e "${GRAY}感谢使用VSCode配置上传脚本！${NC}"